...
def neg_log_likelihood(data, theta, beta, guess, alpha):
    ...
    for i, q in enumerate(user_id): # q represents the actual user_id
        ...
        if g_j > 1:
            g_j = 1
        log_lklihood += c_ij * np.log(g_j + exp) + (1 - c_ij) * np.log(1 - g_j) - np.logaddexp(0, a_j * (theta_i - beta_j))
    # #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return -log_lklihood
...
def update_guess(data, lr, lr2, theta, beta, guess, alpha):
    """ Update theta and beta using gradient descent.
    ...
    for i, q in enumerate(user_id):
        ...
        guess[question_id[i]] += lr2 * (c_ij / (g_j + exp) - (1 - c_ij) / (1 - g_j))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return theta, beta, guess, alpha

def irt(data, val_data, lr, lr2, iterations, iterations2):
    """ Train IRT model.
    ...
    # guess = np.full((max(question_id) + 1,), 0.05)
    ...
    for j in range(iterations2):
        ...
        print(np.amin(guess))
        neg_lld = neg_log_likelihood(data, theta=theta, beta=beta, guess=guess, alpha=alpha)
        train_lld_lst2.append(neg_lld)
        val_lld_lst2.append(neg_log_likelihood(val_data, theta, beta, guess=guess, alpha=alpha))

        score = evaluate(data=val_data, theta=theta, beta=beta, guess=guess, alpha=alpha)
        val_acc_lst2.append(score)
        print("NLLK: {} \t Score: {}".format(neg_lld, score))
        theta, beta, guess, alpha = update_guess(data, lr, lr2, theta, beta, guess, alpha)

    return theta, beta, guess, alpha, val_acc_lst, val_lld_lst, train_lld_lst, \
        val_acc_lst2, val_lld_lst2, train_lld_lst2
...
def main():
    ...
    iteration = 40
    iteration2 = 25
    theta, beta, guess, alpha, val_acc_lst, val_lld_lst, train_lld_lst, val_acc_lst2, val_lld_lst2, train_lld_lst2 = \
        irt(train_data, val_data, 0.005, 0.0001, iteration, iteration2)
    ...
    iteration2_lst = range(1, iteration2+1)
    plt.plot(iteration2_lst, val_lld_lst2)
    plt.xlabel('iteration')
    plt.ylabel('negative log-likelihood')
    plt.title('validation nlld vs iteration')
    plt.show()
